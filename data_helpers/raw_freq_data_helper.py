...
class RawFreqDataHelper:
    ...
    def AddRawFreqData(self, raw_freq, updateExisting=True):
        if(raw_freq != None):
            ...
            queriedEvent = self.session.query(RawFreq).filter(
                RawFreq.data_time == raw_freq.data_time).first()
            if not queriedEvent:
    ...
    def GetRawFreqData(self, args_obj):
        ...
        queriedEvents = self.session.query(RawFreq).filter(RawFreq.data_time.between(
            from_time, to_time)).order_by(RawFreq.data_time.desc())
